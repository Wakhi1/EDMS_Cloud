...
router.get('/', requireModuleAccess('departments'), asyncHandler(async (req, res) => {
  const [rows] = await pool.query('SELECT * FROM departments WHERE company_id = ? ORDER BY name', [req.user.companyId]);
  return ok(res, rows);
}));
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [existing] = await pool.query('SELECT id FROM departments WHERE company_id = ? AND name = ?', [req.user.companyId, name]);
    if (existing.length) return fail(res, 'A department with this name already exists', 409);

    const [result] = await pool.query('INSERT INTO departments (company_id, name, description) VALUES (?, ?, ?)', [req.user.companyId, name, description || null]);
    await logAudit({ userId: req.user.id, companyId: req.user.companyId, action: 'Create', recordType: 'department', recordId: result.insertId, detail: name, ip: req.ip });
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [[dept]] = await pool.query('SELECT id FROM departments WHERE id = ? AND company_id = ?', [req.params.id, req.user.companyId]);
    ...
    await logAudit({ userId: req.user.id, companyId: req.user.companyId, action: 'Edit', recordType: 'department', recordId: req.params.id, ip: req.ip });
...
router.delete('/:id', requireModuleAccess('departments', true), asyncHandler(async (req, res) => {
  const [[dept]] = await pool.query('SELECT id, name FROM departments WHERE id = ? AND company_id = ?', [req.params.id, req.user.companyId]);
  if (!dept) return fail(res, 'Department not found', 404);
  ...
  await logAudit({ userId: req.user.id, companyId: req.user.companyId, action: 'Delete', recordType: 'department', recordId: req.params.id, detail: dept.name, ip: req.ip });
